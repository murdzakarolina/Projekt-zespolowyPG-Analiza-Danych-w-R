...
gghistostats(
  ...
  test.value = 5, # Przykładowa wartość testowa
  test.value = 5, 
  title = "P1: Rozkład czasu w social mediach"
)
```
...
```ggbetweenstats(
  ...
  title = "P3: Płeć a czas w social mediach"
)
```

Hipotezy: H_0: = \mu_{K} = \mu_{M}; H_1: \mu_{K} \neq \mu_{M}
Poziom istotności: alpha = 0,05.
...
```ggbetweenstats(
  ...
  y = Avg_Daily_Usage_Hours,
  title = "P7: Status związku a czas w social mediach"
)
```
Wniosek: Status związku nie ma żadnego istotnego wpływu na to, ile czasu studenci spędzają w mediach społecznościowych. Różnice między grupami są minimalne i wynikają wyłącznie z przypadku.