...
```{r pobieranie_danychh, include=FALSE}
...
```{r sekcja5_start_final, echo=TRUE, message=FALSE, warning=FALSE}
# 1. Reset ustawień graficznych
...
# 2. Pobieranie danych 
url_pliku <- "(https://github.com/murdzakarolina/Projekt-zespolowyPG-Analiza-Danych-w-R/blob/main/data/Students_Social_Media_Addiction.csv?raw=true)"

if(!file.exists("dane5.csv")) {
   download.file(url_pliku, destfile = "dane5.csv", mode="wb")
}
...
gghistostats(data = dane5, x = Avg_Daily_Usage_Hours, test.value = 5, title = "P1: Rozkład czasu w social mediach")
```
...
```{r}
ggpiestats(data = dane5, x = Most_Used_Platform, package = "ggthemes", palette = "Tableau_20", title = "P2: Dominująca platforma")
```
Hipotezy: H_0: Rozkład platform jest równomierny; H_1: Istnieje platforma dominująca.
Poziom istotności: alpha = 0,05.
